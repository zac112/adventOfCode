...
class Cookie:
    ...
    def __init__(self,name, cap, dur, fla, tex):
        self.name = name
        self.capacity = cap
        self.durability = dur
        self.flavor = fla
...
with open("../../data/15.txt", "r") as f:
    for line in f:
        split = line.split(" ")
        cookies.append(Cookie(split[0][:-1], int(split[2][:-1]), int(split[4][:-1]), int(split[6][:-1]), int(split[8][:-1])))
    ...
    for b in range (total-a):        
        for c in range (total-a-b):            
            for d in range (total-a-b-c):                                
                ...
                maxScore = max(reduce((lambda a,b:a*b),vals),maxScore)
print(maxScore)
