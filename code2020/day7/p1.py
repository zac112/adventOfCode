...
import re
adjacency = {}
regex = "(.*?) bags contain (\d+ .* bag[s,.]{1,2})*"
adjacency = {m[0]:re.split("bag[s]?[,.]",m[1]) for m in re.findall(regex,data)}

for k, v in adjacency.items():
    v = [x.strip() for x in v]
    adjacency[k] = [ (x[x.index(" ")+1:], int(x[:x.index(" ")]) ) for x in v if x!=""]

print(adjacency)

def findIndex(adjacency, index, goal, visited, path):
    visited.append(index)

    if goal in [x[0] for x in adjacency[index]]:
        path.append(list(visited))

    for i, dist in adjacency[index]:        
        findIndex(adjacency, i, goal, visited, path)
...
path = []
for k in adjacency:    
    findIndex(adjacency, k, "shiny gold",[], path)    
nodes = set([p[0] for p in path])
print(nodes)
print(len(nodes))
