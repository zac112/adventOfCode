...
data = data.replace("peanuts","peanus")
import re
...
for line in data.splitlines():
    m= re.search(allergenRe,line)
    #print(m.group(1),m.group(2))
    a = m.group(2).strip()
    i = m.group(1).strip()
    allIngredients.extend(i.split(" "))
    
    if m.group(2).strip() in ingredients:
        ingredients[a] = ingredients[a] & set(i.split(" "))
    else:
        ingredients[a] = set(i.split(" "))
...
for a in allerg:
    for k,v in ingredients.items():
        if a in k:
            allergens.setdefault(a,[]).extend(v)
...
allIng = Counter(allIngredients)
print(allIng)
for k,v in allergens.items():
    for x in v:
        while x in allIngredients:
...
print("Solution p2:",[v for k,v in sorted(allergens.items(), key=lambda a:a[0])])
#high; 2116
