data="""LLLLLLLLLLL..LLLLLL.LLLLL.LLLLLLLLLLLLLLL.LLLLLLLLLLLLLLLLLLLLLLLL.LLLLLL.LL.LLLLLLLL..LLLLL
...
area = [[x for x in line] for line in data.splitlines()]

def applyRules(area, x,y):
    if area[x][y] == ".":
        return "."
    ...
    for i in range(-1,2):
        for j in range(-1,2):
            ...
            x_n=x+i
            y_n=y+j
            while not( x_n < 0 or y_n < 0 or x_n >= len(area) or y_n >= len(area[x])):
                try:
                    if area[x_n][y_n]=="L":
                        break
                    if area[x_n][y_n]=="#":
                    ...
                    print(x,y,i,j,x_n,y_n, len(area),len(area[x]))
                x_n=x_n+i
                y_n=y_n+j  
...
def getNextGeneration(area):
    result = []
    for y, line in enumerate(area):
        result.append([])
        for x, cell in enumerate(line):
            result[y].append(applyRules(area,y,x))
    return result
