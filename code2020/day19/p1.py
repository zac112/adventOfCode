...
for line in data.split("\n"):
    if ":" in line:
        ...
        for val in vals.replace('"',"").split(" ")[1:]:
            if val == "|":
            ...
            if val in 'ab':
                ...
                continue
            rules.setdefault(key,[]).append(rules.setdefault(val,[]))
        #rules[key] = vals.replace('"',"").split(" ")[1:]
    else:
        messages.append(line)

#print(rules)
exp = re.sub("[', ]", "", str(rules['0']))
...
def expand(expression):
    ...
    for e in re.findall("(\d+)", expression):
        ...
        print()
        expression = re.sub(e,f"({rules[int(e)]})", expression)
