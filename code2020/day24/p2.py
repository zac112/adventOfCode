...
def countBlackNeighbors(coord, tiles):
    x,y = coord
    neighbors = 0
    for d,v in directions.items():
        x1,y1 = v
        if (x+x1,y+y1) in tiles and tiles[(x+x1,y+y1)]==1:
            neighbors += 1
    return neighbors

def newGen(tiles):
    tiles = {a:b for a,b in tiles.items() if b==1}
    result = {}
    for coord in tiles:
        x,y = coord
        for d,v in directions.items():
            x1,y1 = v
            nc = (x+x1,y+y1)
            c = countBlackNeighbors(nc, tiles)
            if c == 2:
                result[nc] = 1
            if c == 1 and nc in tiles:
                result[nc] = 1
    return result
