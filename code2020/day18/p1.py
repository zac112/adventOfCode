...
def getSubExpR(exp):
    ...
    if subExp[-1] == ")":
        ...
        while par > 0:            
            ...
            if left == "(":
                par -= 1
            subExp.appendleft(left)

        subExp.popleft()
        subExp.pop()
        #print("returning",subExp)
        return subExp
    else:        
        return subExp.pop()
...
def evaluate(exp):
    ...
    try:
        ...
        right = evaluate(getSubExpR(exp))

    try:
        oper = exp.pop()
    except:
        #print("Done, exp",exp,right)
        return right
    left = evaluate(exp)

    #print("Rec, exp",exp,left,oper,right)
    return opers[oper](int(left),int(right))    

e = "1 * 2 + 3"
e = "2 * 3 + (4 * 5)"
e = "5 + (8 * 3 + 9 + 3 * 4 * 3)"
print(e)
print(evaluate(parse(e)))

s = 0
for exp in data.splitlines():
    s += evaluate(parse(exp))
print("Sum of expressions",s)
