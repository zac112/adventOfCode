...
'iyr':"201[0-9]|2020",
'eyr':"202[0-9]|2030",
'hgt':"(1[5678][0-9]|19[0123])cm|(59|6[0-9]|7[0-6])in",
'hcl':"#[0-9a-f]{6}",
...
def check(d):
    for key,val in d.items():
        if key=='cid':
            continue
        if re.fullmatch(model[key],val) == None:
            return False
    return True

passports = [{b.split(":")[0]:b.split(":")[1] for b in a.split(" ")} for a in data.replace("\n\n","!").replace("\n"," ").replace("!","\n").split("\n")]
passports = [valid for valid in passports if model.keys()== valid.keys()&model.keys() ]
