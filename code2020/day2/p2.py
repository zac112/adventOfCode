...
passwords= [ (x.split(" ")[0],x.split(" ")[1][:-1], x.split(" ")[2]) for x in data.split("\n")]

c = 0
for p in passwords:
    i1 = int(p[0].split("-")[0])-1
    i2 = int(p[0].split("-")[1])-1
    
    if (p[2][i1] == p[1] and p[2][i2] != p[1]) or (p[2][i1] != p[1] and p[2][i2] == p[1]):
        print(p)
        c += 1
print (c)
