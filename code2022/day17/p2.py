...
#while True:
    if piecetype == 0 and pattern_index == 0:
        print(f"piece {i} starts again")
        print(0 if len(stoppedPieces)==0 else max((y for x,y in stoppedPieces))+1)

    if i%10000==0:
        l = 0 if len(stoppedPieces)==0 else max((y for x,y in stoppedPieces))+1
        print(f"Checkpoint {i} {piecetype=} {pattern_index=} {l}")
    start = (2,max((y for x,y in stoppedPieces))+4) if len(stoppedPieces)>0 else (2,3)
    p = Piece(piecetype,i, start)
    ...
    discardPile = []
    for x,y in stoppedPieces:
        if y < start[1]-200:
            discardPile.append((x,y))            

    for pos in discardPile:
        stoppedPieces.discard(pos)    
    i += 1
